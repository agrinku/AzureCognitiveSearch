locals {
  eventsdl_index_name       = "${local.search_service_name}eventsdlindex"
  eventsdl_data_source_name = "${local.search_service_name}eventsdldata"
  eventsdl_indexer_name     = "${local.search_service_name}eventsdlindexer"
...
resource "null_resource" "eventsdl_search_data_source" {
  ...
  provisioner "local-exec" {
    command = <<EOT
          $uri ="https://${local.search_service_name}.search.windows.net:443/datasources/${local.eventsdl_data_source_name}?api-version=2020-06-30"
...
resource "null_resource" "eventsdl_search_index" {
  ...
  provisioner "local-exec" {
    command     = <<EOT
    $uri = "https://${local.search_service_name}.search.windows.net:443/indexes/${local.eventsdl_index_name}?api-version=2020-06-30"
    $headers    = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
    $headers.Add("Content-Type", "application/json")
    $headers.Add("api-key", "${azurerm_search_service.instance.primary_key}")
    $headers.Add("host", "${local.search_service_name}.search.windows.net:443")
...
resource "null_resource" "eventsdl_search_indexer" {
 triggers = {
  ...
  provisioner "local-exec"{
    command     = <<EOT
        $uri = "https://${local.search_service_name}.search.windows.net:443/indexers/${local.eventsdl_indexer_name}?api-version=2020-06-30"
        $headers    = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
        $headers.Add("Content-Type", "application/json")
        $headers.Add("api-key", "${azurerm_search_service.instance.primary_key}")
        $headers.Add("host", "${local.search_service_name}.search.windows.net:443")
