locals {
  eventcorrelation_index_name       = "${local.search_service_name}eventcorrelationindex"
  eventcorrelation_data_source_name = "${local.search_service_name}eventcorrelationdata"
  eventcorrelation_indexer_name     = "${local.search_service_name}eventcorrelationindexer"
...
resource "null_resource" "eventcorrelation_search_data_source" {
  ...
  provisioner "local-exec" {
    command = <<EOT
          $uri ="https://${local.search_service_name}.search.windows.net:443/datasources/${local.eventcorrelation_data_source_name}?api-version=2020-06-30"
          $headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
          $headers.Add("Content-Type", "application/json")
          $headers.Add("api-key", "${azurerm_search_service.instance.primary_key}")
          $headers.Add("host", "${local.search_service_name}.search.windows.net:443")
          $headers.Add("Accept", "*/*")
          $body = @'
		{
...
resource "null_resource" "eventcorrelation_search_index" {
  ...
  provisioner "local-exec" {
    command     = <<EOT
    $uri = "https://${local.search_service_name}.search.windows.net:443/indexes/${local.eventcorrelation_index_name}?api-version=2020-06-30"
    $headers    = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
    $headers.Add("Content-Type", "application/json")
    $headers.Add("api-key", "${azurerm_search_service.instance.primary_key}")
    $headers.Add("host", "${local.search_service_name}.search.windows.net:443")
 ...
 triggers = {
  ...
  provisioner "local-exec" {
    command     = <<EOT
       ...
       $uri = "https://${local.search_service_name}.search.windows.net:443/indexers/${local.eventcorrelation_indexer_name}?api-version=2020-06-30"
        $headers    = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
        $headers.Add("Content-Type", "application/json")
        $headers.Add("api-key", "${azurerm_search_service.instance.primary_key}")
        $headers.Add("host", "${local.search_service_name}.search.windows.net:443")
